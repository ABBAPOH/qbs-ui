...
{
    const auto jsonArray = array.toArray();
    QStringList list;
    auto transformer = [](const QJsonValue &v) { return v.toString(); };
    std::transform(jsonArray.cbegin(), jsonArray.end(), std::back_inserter(list), transformer);
    return list;
}
...
void QbsSession::initialize()
{
    ...
    connect(d->qbsProcess, &QProcess::readyReadStandardError, this, [this] {
        qCDebug(lcQbsSession) << "[qbs stderr]: " << d->qbsProcess->readAllStandardError();
    });
    ...
    connect(d->packetReader, &PacketReader::errorOccurred, this, [this](const QString &msg) {
        qCDebug(lcQbsSession) << "session error" << msg;
        setError(Error::ProtocolError);
    });
    ...
    d->qbsProcess->start(qbsExe, {"session"});
}
...
//QbsSession::BuildGraphInfo QbsSession::getBuildGraphInfo(const QString &bgFilePath,
//                                                         const QStringList &requestedProperties)
//{
//    const QFileInfo bgFi(bgFilePath);
...
//        }
        ...
        emit processResult(
                    packet.value("executable-file-path").toString(),
                    arrayToStringList(packet.value("arguments")),
                    packet.value("working-directory").toString(),
...
ErrorInfoItem::ErrorInfoItem(const QJsonObject &data)
{
    ...
    filePath = locationData.value("file-path").toString();
...
{
    QStringList list;
    auto transformer = [](const ErrorInfoItem &i) { return i.toString(); };
    std::transform(items.cbegin(), items.end(), std::back_inserter(list), transformer);
    return list.join('\n');
}
...
Location locationFromObject(const QJsonObject &o)
{
    ...
    return Location(loc.value("file-path").toString(), loc.value("line").toInt());
}

Q_LOGGING_CATEGORY(lcQbsSession, "org.qbs-ui.qbsSession");
